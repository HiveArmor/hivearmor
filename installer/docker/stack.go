...
func StackUP(tag string) error {
	...
	if config.ConnectedToInternet {
		...
		for _, service := range compose.Services {
			image := strings.ReplaceAll(*service.Image, "${HIVEARMOR_TAG}", tag)
	...
	env := []string{"HIVEARMOR_TAG=" + tag}
	if err := utils.RunEnvCmd(env, "docker", "stack", "deploy", "-c", filepath.Join(utils.GetMyPath(), "compose.yml"), "hivearmor"); err != nil {
		return err
	}
