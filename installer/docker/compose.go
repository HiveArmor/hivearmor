...
func (c *Compose) Populate(conf *config.Config, stack *StackConfig) error {
	...
	c.Services["agentmanager"] = Service{
		Image: utils.PointerOf[string]("ghcr.io/hivearmor/agent-manager:${HIVEARMOR_TAG}"),
		Volumes: []string{
			stack.Cert + ":/cert",
	...
	c.Services["postgres"] = Service{
		Image: utils.PointerOf[string]("ghcr.io/hivearmor/postgres:latest"),
		Environment: []string{
	...
	c.Services["frontend"] = Service{
		Image: utils.PointerOf[string]("ghcr.io/hivearmor/frontend:${HIVEARMOR_TAG}"),
		DependsOn: []string{
			"backend",
			"filebrowser",
		},
	...
	c.Services["backend"] = Service{
		Image: utils.PointerOf[string]("ghcr.io/hivearmor/backend:${HIVEARMOR_TAG}"),
		DependsOn: []string{
			"postgres",
			"node1",
			"agentmanager",
		...
		Volumes: []string{
			stack.DataSources + ":/etc/hivearmor",
			conf.UpdatesFolder + ":/updates",
			stack.Cert + ":/cert:ro",
		},
		Logging: &dLogging,
	...
	c.Services["event-processor-worker"] = Service{
		Image: utils.PointerOf[string]("ghcr.io/hivearmor/eventprocessor:${HIVEARMOR_TAG}"),
		...
		Volumes: []string{
			...
			utils.MakeDir(0777, stack.EventsEngineWorkdir, "rules") + ":/workdir/rules/hivearmor",
			stack.Cert + ":/cert",
			conf.UpdatesFolder + ":/updates",
		},
		Environment: []string{
	...
	c.Services["node1"] = Service{
		Image: utils.PointerOf[string]("ghcr.io/hivearmor/opensearch:latest"),
		Volumes: []string{
	...
	c.Services["user-auditor"] = Service{
		Image: utils.PointerOf[string]("ghcr.io/hivearmor/user-auditor:${HIVEARMOR_TAG}"),
		DependsOn: []string{
			"postgres",
			"node1",
		},
	...
	c.Services["web-pdf"] = Service{
		Image: utils.PointerOf[string]("ghcr.io/hivearmor/web-pdf:${HIVEARMOR_TAG}"),
		Volumes: []string{
