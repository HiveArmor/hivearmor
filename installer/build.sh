...
export GOOS=linux
export GOARCH=amd64
export GOPRIVATE=github.com/hivearmor
export GONOPROXY=github.com/hivearmor
export GONOSUMDB=github.com/hivearmor

echo "Building V11 Installer for production release"

# Execute build with ldflags
go build -o installer -v -ldflags "\
-X 'github.com/hivearmor/installer/config.DEFAULT_BRANCH=${DEFAULT_BRANCH}' \
-X 'github.com/hivearmor/installer/config.INSTALLER_VERSION=${INSTALLER_VERSION}' \
