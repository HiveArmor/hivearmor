...
- **DELIVERED (A2-4):** `validateTenantScope(...)` now guards all three metadata reads via the shared `TenantScopeGuard`, identical to the query endpoints (`search`/`count`/`generic-search`/`search/csv`/`search/sql`). Non-MSSP mode stays permissive (backward-compat); MSSP mode denies any pattern outside the caller's alert/log prefix with `TenantScopeViolationException`. Covered by 8 new cases in `ElasticsearchResourceTenantValidationTest` (own-allowed / other-denied / non-MSSP-allowed per endpoint). `getAllIndexes`/`deleteIndex`/`cluster/status` are admin index-management (index NAMES + cluster health, not per-tenant document/field reads) — out of A2-4's metadata-leak scope; their access control is admin-`@PreAuthorize`, not tenant-pattern scoping.
