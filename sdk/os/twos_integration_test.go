...
func TestEntityWorkflow(t *testing.T) {
	...
	index := twos.BuildGenericCurrentDayIndex(twos.CommentPrefix)
	...
	t.Run("IndexDoc", func(t *testing.T) {
		...
		err = twos.RefreshIndex(context.Background(), twos.BuildGenericIndexPattern(twos.CommentPrefix))
		if err != nil {
			t.Error(err)
		}
	})
	...
	t.Run("SearchIn", func(t *testing.T) {
		...
		resp, err := req.SearchIn(context.Background(), []string{twos.BuildGenericIndexPattern(twos.CommentPrefix)}, []string{"public"})
		if err != nil {
			t.Error(err)
		}
