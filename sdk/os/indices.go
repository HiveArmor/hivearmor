...
// BuildGenericIndexPattern returns a string representing a generic index pattern
// based on the given elements joined with "-" and suffixed with "*".
//
// This is the generic entity-store variant. For HiveArmor log/alert index patterns,
// use BuildIndexPattern (single-arg, produces the HiveArmor wildcard pattern) instead.
//
// Example: BuildGenericIndexPattern("comment") → "comment-*".
func BuildGenericIndexPattern(elements ...string) string {
	elements = append(elements, "*")
	return strings.Join(elements, "-")
}

// BuildGenericCurrentDayIndex returns a generic current-day index name formatted
// as "<elements-joined-with->-YYYY-MM-DD" (dashes, not dots).
//
// This is the generic entity-store variant. For HiveArmor log/alert index names,
// use BuildCurrentDayIndex (single-arg, produces the HiveArmor daily index name) instead.
//
// Example: BuildGenericCurrentDayIndex("comment") → "comment-2026-07-25".
func BuildGenericCurrentDayIndex(elements ...string) string {
