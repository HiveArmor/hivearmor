...
// regexValueMatch evaluates a regular expression against an already-resolved
// string. It is intentionally separate from regexMatch, whose first argument is
// a JSON field path, so missing fields cannot accidentally be interpreted as
// literal values.
func (c *CELCache) regexValueMatch() cel.EnvOption {
	return cel.Function("regexValueMatch", cel.Overload(
		"string_string_regexValueMatch_bool",
		[]*cel.Type{cel.StringType, cel.StringType},
		cel.BoolType,
		cel.BinaryBinding(func(value ref.Val, pattern ref.Val) ref.Val {
			re, err := rCache.Get(pattern.Value().(string))
			if err != nil {
				return types.False
			}
			return types.Bool(re.MatchString(value.Value().(string)))
		}),
	))
}

// toLower returns a lowercase copy of a string expression. Unlike the
// field-path helpers above, it accepts the value produced by safe(), so rules
// can normalize untrusted telemetry before matching it.
func (c *CELCache) toLower() cel.EnvOption {
	return cel.Function("toLower", cel.Overload(
		"string_toLower_string",
		[]*cel.Type{cel.StringType},
		cel.StringType,
		cel.UnaryBinding(func(value ref.Val) ref.Val {
			raw, ok := value.Value().(string)
			if !ok {
				return types.NewErr("toLower requires a string")
			}
			return types.String(strings.ToLower(raw))
		}),
	))
}
