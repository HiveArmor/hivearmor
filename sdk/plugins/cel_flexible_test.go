...
func TestCELFlexibleOverloads(t *testing.T) {
	...
	tests := []struct {
		...
		{"regexMatch_missing_field", `regexMatch("missing.field", ".*")`, false},
		{"toLower_safe_value", `toLower(safe("text", "")) == "hello world"`, true},
		{"toLower_regex", `regexMatch(toLower(safe("text", "")), "^hello")`, true},
		{"safe_regex", `regexMatch(safe("text", ""), "World$")`, true},
