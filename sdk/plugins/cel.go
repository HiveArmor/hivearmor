...
func (c *CELCache) transformExpression(expression string) string {
	// Field helpers such as regexMatch normally accept a JSON field path. Rules
	// also compose helpers, e.g. regexMatch(toLower(safe("log.message", "")),
	// "pattern"). Preserve that authored syntax while routing resolved values to
	// the value overload before adding the implicit _data_ argument.
	expression = strings.ReplaceAll(expression, "regexMatch(toLower(", "regexValueMatch(toLower(")
	expression = strings.ReplaceAll(expression, "regexMatch(safe(", "regexValueMatch(safe(")
