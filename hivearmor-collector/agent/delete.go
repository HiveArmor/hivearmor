...
func DeleteAgent(cnf *config.Config) error {
	...
	if err != nil {
		utils.Logger.ErrorF("error removing HiveArmor Collector from Agent Manager %v", err)
	}

	utils.Logger.LogF(100, "HiveArmor Collector removed successfully from agent manager")
	return nil
}
