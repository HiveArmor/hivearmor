...
import (
	...
	"github.com/hivearmor/hivearmor-collector/serv"
	"github.com/hivearmor/hivearmor-collector/updates"
...
func main() {
	...
	if len(os.Args) > 1 {
		...
		isInstalled, err := utils.CheckIfServiceIsInstalled("HiveArmorCollector")
		if err != nil {
			fmt.Println("Error checking if service is installed: ", err)
			os.Exit(1)
		}
		if arg != "install" && !isInstalled {
			fmt.Println("HiveArmorCollector service is not installed")
			os.Exit(1)
		} else if arg == "install" && isInstalled {
			fmt.Println("HiveArmorCollector service is already installed")
			os.Exit(1)
		}
		...
		case "install":
			utils.PrintBanner()
			fmt.Println("Installing HiveArmorCollector service ...")
			...
			serv.InstallService()
			fmt.Println("[OK]")
			fmt.Println("HiveArmorCollector service installed correctly")
		...
		case "uninstall":
			fmt.Println("Uninstalling HiveArmorCollector service ...")
			...
			fmt.Println("[OK]")
			fmt.Println("HiveArmorCollector service uninstalled correctly")
			os.Exit(1)
		case "help":
			Help()
		default:
...
func Help() {
	fmt.Println("### HiveArmor Collector ###")
	fmt.Println("Usage:")
	fmt.Println("  To run the service:                     ./hivearmor_collector run")
	fmt.Println("  To install the service:                 ./hivearmor_collector install <server> <connection-key> <yes|no> [tenant-id]")
	fmt.Println("  Or set HA_TENANT_ID=<id> before install")
	fmt.Println("  To change log retention:                ./hivearmor_collector change-retention <new_retention>")
	fmt.Println("  To clean old logs:                      ./hivearmor_collector clean-logs")
	fmt.Println("  To uninstall the service:               ./hivearmor_collector uninstall")
	fmt.Println("  To debug HiveArmor installation:         ./hivearmor_collector debug-hivearmor")
	fmt.Println("  For help (this message):                ./hivearmor_collector help")
	fmt.Println()
	fmt.Println("Options:")
	fmt.Println("  run                      Run the HiveArmorCollector service")
	fmt.Println("  install                  Install the HiveArmorCollector service")
	...
	fmt.Println("  uninstall                Uninstall the HiveArmorCollector service")
