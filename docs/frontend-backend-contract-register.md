...
Reconciled **2026-08-25 09:15:00 IST (UTC+05:30)** against `HaAgentPolicyResource`, `AgentPolicyResource` (`/api/agent-policies`), and `AgentPolicyStateDTO` / `hive_agent_policy_state`. Orphan inventory gap: delivery/enforcement evidence incomplete. **STAGING CANDIDATE** — not `PRODUCTION READY`. Does not claim agents enforce policies on host without a live-verified agent evidence path.

| ID | Status | Existing capability | Remaining requirement |
|---|---|---|---|
| POL-001 | `PARTIAL` | `GET /api/ha-edr/policies/{id}/enforcement` returns assigned agent IDs plus any `AgentPolicyStateDTO` rows for the policy id; `evidenceAvailability` is only `unavailable` or `partial` with an honesty note. Frontend honesty banner + evidence drawer. | Live-verified agent apply/ack path, desired/applied version continuity across ha-edr and agent-manager policy stores, and production host-enforcement proof. Never invent Neo4j/Kafka for this gap. |
...
| POL-003 | `MISSING` | `POST /api/agent-policies/report-state` is Admin JWT-only; no production agent INTERNAL_KEY / enrolled-device evidence path is wired into the ha-edr UI. | Authenticated agent state reporting with idempotency, drift receipts, and UI that still refuses complete/green claims until LIVE VERIFIED. |

Frontend integration recorded **2026-08-25 09:15:00 IST (UTC+05:30)**: `/edr/policies` nav/AuthGuard/page gates accept Analyst read; SOC Manager may mutate; enforcement evidence surfaces real state fields or explicit unavailable/partial. No PRODUCTION READY claim.
