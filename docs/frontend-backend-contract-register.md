...
| TI-004 | `PARTIAL` | Admin TAXII/MISP CRUD + stats; manual sync returns thin `ThreatFeedSyncReceipt` (`receiptId`, `lastSyncAt`, `status`, `iocCount`, `failedReason`); TAXII failures persist `lastSyncStatus=ERROR` + `lastSyncAt` (no fake zero-IOC success). | Durable sync job ledger, credential write-only aliases, tenant scope, TLP/marking enforcement evidence, MISP persisted status column, and governed source onboarding remain incomplete. |

Frontend + backend integration recorded **2026-08-25 09:02:14 IST (UTC+05:30)**: TI-002 explicit Analyst/SOC Manager on feed list/get/stats; TI-003 legacy v1 `@PreAuthorize` harden (no deprecation headers); TI-004 sync receipt JSON on TAXII/MISP sync with Admin toast honesty. STAGING CANDIDATE only. Not live-vendor verified. Not `PRODUCTION READY`.
