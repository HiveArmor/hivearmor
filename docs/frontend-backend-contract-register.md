...
|---|---|---|
| INV-LV01 | `PARTIAL` | `POST/GET/PUT/DELETE /api/ha-investigation-sessions` and `GET /{id}` are implemented, role-gated, tenant-scoped, and owner-scoped; update requires the returned optimistic `version`. Lists are offset-paged and server-bounded to 100. Missing: server search/status/assignment filters, saved views, same-snapshot summary/facets, stable cursor pagination, freshness/partial-source state, and permission descriptors. **STAGING CANDIDATE (2026-08-25):** agentic triage `INVESTIGATE` may soft-create a session and soft-pin the alert as an `ALERT` session item (`itemRef` = alert id) when resolvable; ledger records `sessionId`/`sessionStatus` and `sessionItemId`/`sessionItemType` (or sanitized pin error); always `stub=true`; no Neo4j/attack-path; no auto-convert to incident. Not `PRODUCTION READY`. |
...
Reconciled **2026-08-25 09:02:14 IST (UTC+05:30)** against secured `HaThreatIntelResource`, `HaThreatIntelStatsResource`, `HaTaxiiFeedResource`, `HaMispFeedResource`, and legacy `ThreatIntelResource` under `/api/v1/threat-intel`. Depth slice after route inventory (`docs/ai-handoff/research/orphan-operational-workflows-inventory.md`). Not `PRODUCTION READY`.

| ID | Status | Existing capability | Remaining requirement |
|---|---|---|---|
| TI-001 | `PARTIAL` | Frontend hub reads feeds/IOCs/lookup; Admin-gated feed enable/sync; SOC Manager page gate + `GET /ha-threat-intel/stats` on `/intelligence`. | Cursor/snapshot/freshness on IOC browse, cancellable lookup budgets, progressive indicator detail, hunt/alert pivots remain incomplete. |
| TI-002 | `PARTIAL` | Feed list/get + stats authorize Admin\|User\|Analyst\|SOC Manager explicitly; lookup/IOC browse unchanged; feed mutate/sync stay Admin-only. | Immutable mutation audit evidence and least-privilege without ROLE_USER remain open for broader SOC roles. |
| TI-003 | `PARTIAL` | Legacy `/api/v1/threat-intel` hardened in place with the same `@PreAuthorize` ROLE_ matrix as the ha-threat-intel successor. Frontend-v3 does not call it. | Consumer telemetry, cutover evidence, then `Deprecation`/`Sunset`/successor `Link`. Do not mark deprecated from frontend non-use alone. |
| TI-004 | `PARTIAL` | Admin TAXII/MISP CRUD + stats; thin `ThreatFeedSyncReceipt` on sync; TAXII + MISP persist `lastSyncStatus` (`OK`/`ERROR`) + `lastSyncAt`; Admin Status column + bounded Last Sync (`>30d` cap). | Durable sync job ledger, credential write-only aliases, tenant scope, TLP/marking enforcement evidence, and governed source onboarding remain incomplete. |

Frontend + backend integration recorded **2026-08-25 11:45:00 IST (UTC+05:30)**: TI-004 optional depth — MISP `last_sync_status` Liquibase + entity/service/resource persistence parity with TAXII; Admin MISP Status column; shared bounded `lastSyncAt` formatter. No Deprecation/Sunset on `/api/v1/threat-intel`. STAGING CANDIDATE only. Not live-vendor verified. Not `PRODUCTION READY`.

Prior integration **2026-08-25 09:02:14 IST (UTC+05:30)**: TI-002 explicit Analyst/SOC Manager on feed list/get/stats; TI-003 legacy v1 `@PreAuthorize` harden (no deprecation headers); TI-004 sync receipt JSON on TAXII/MISP sync with Admin toast honesty.

## Agent policy enforcement evidence (`/edr/policies`)

Reconciled **2026-08-25 11:45:00 IST (UTC+05:30)** against `HaAgentPolicyResource`, `AgentPolicyResource` (`/api/agent-policies`), and `AgentPolicyStateDTO` / `hive_agent_policy_state`. Orphan inventory gap: delivery/enforcement evidence incomplete. **STAGING CANDIDATE** — not `PRODUCTION READY`. Does not claim agents enforce policies on host without a live-verified agent evidence path.

| ID | Status | Existing capability | Remaining requirement |
|---|---|---|---|
| POL-001 | `PARTIAL` | `GET /api/ha-edr/policies/{id}/enforcement` returns assigned agent IDs plus any `AgentPolicyStateDTO` rows for the policy id; `evidenceAvailability` is only `unavailable` or `partial` with an honesty note; `applyAckPathAvailable` is true only when `appliedVersion` or `lastAppliedAt` is present. Frontend honesty banner + evidence drawer. | Live-verified agent apply/ack path, desired/applied version continuity across ha-edr and agent-manager policy stores, and production host-enforcement proof. Never invent Neo4j/Kafka for this gap. |
| POL-002 | `PARTIAL` | ha-edr policy reads authorize Admin\|SOC Manager\|Analyst; mutations Admin\|SOC Manager. Agent-manager `/api/agent-policies` reads similarly; mutations Admin\|SOC Manager. | Immutable audit of assign/push, tenant scope, and Analyst read without over-broad ROLE_USER co-assignment on unrelated surfaces. |
| POL-003 | `PARTIAL` | After #48 enforcement evidence: UI+API honesty refuses green “enforced on host”. When `AgentPolicyStateDTO` lacks `appliedVersion`/`lastAppliedAt`, `evidenceAvailability=unavailable`, `applyAckPathAvailable=false`, and notes say **apply/ack path unavailable**. Bare `state`/`desiredVersion` rows are not treated as ack. | Authenticated agent INTERNAL_KEY / enrolled-device / gRPC apply+ack with idempotency and drift receipts; flip host-enforcement LIVE VERIFIED only with proof. `POST /api/agent-policies/report-state` remains Admin JWT-only. |

Frontend integration recorded **2026-08-25 11:45:00 IST (UTC+05:30)**: `/edr/policies` surfaces apply/ack path unavailable when ack fields are missing; evidence table shows lastCheckedAt/driftDetails; never claims complete/green host enforcement. No PRODUCTION READY claim.
...
| 2026-08-25 09:02:14 IST (UTC+05:30) | `/api/ha-threat-intel/feeds*`, `/api/ha-threat-intel/stats`, `/api/v1/threat-intel`, TAXII/MISP sync | TI depth STAGING CANDIDATE: TI-002 explicit Analyst/SOC Manager on feed list/get/stats; TI-003 legacy v1 `@PreAuthorize` harden (no Deprecation headers); TI-004 `ThreatFeedSyncReceipt` on TAXII/MISP sync with honest ERROR + lastSyncAt. Not PRODUCTION READY. |
| 2026-08-25 11:45:00 IST (UTC+05:30) | `/api/ha-threat-intel/misp-feeds*`, MISP sync, `/admin/threat-intel` | TI-004 optional depth: MISP `last_sync_status` persisted (OK/ERROR) with TAXII parity; Admin Status column; bounded Last Sync display. No v1 Deprecation/Sunset. STAGING CANDIDATE — not PRODUCTION READY. |
...
| 2026-08-25 09:02:07 IST (UTC+05:30) | SOC-AI agentic `INVESTIGATE`, `/api/ha-investigation-sessions` | STAGING CANDIDATE soft link: when INVESTIGATE runs and the alert is OpenSearch-resolvable, optionally create an investigation session titled from alert name/id and record `sessionId`/`sessionStatus` on triage ledger metadata while keeping `stub=true`. Soft-fail keeps stub honesty with sanitized `sessionLinkError`. No Neo4j/attack-path, no auto-convert to incident, `openHypotheses` remains empty. Updated `INV-LV01` note. Not `PRODUCTION READY`. |
| 2026-08-25 11:45:00 IST (UTC+05:30) | SOC-AI agentic `INVESTIGATE`, `/api/ha-investigation-sessions/{id}/items` | STAGING CANDIDATE soft pin follow-on to #45: after successful soft session create, soft-pin the alert as `ALERT` session item (`itemRef` = alert id) via `pinItem`; ledger records `sessionItemPinned` + `sessionItemId`/`sessionItemType` or sanitized `sessionItemPinError` without undoing `sessionLinked`; always `stub=true`; no Neo4j/attack-path; no auto-convert. Updated `INV-LV01`. Not `PRODUCTION READY`. |
...
| 2026-08-25 09:15:00 IST (UTC+05:30) | `/edr/policies`, `/api/ha-edr/policies*`, `/api/ha-edr/policies/{id}/enforcement`, `/api/agent-policies` | Orphan policies honesty: secured enforcement evidence read wires `AgentPolicyStateDTO` fields with unavailable/partial only; Analyst read + Admin/SOC Manager mutate; UI banner/drawer refuse fictional host-enforcement checks. Recorded `POL-001`–`POL-003`. STAGING CANDIDATE — not PRODUCTION READY. |
| 2026-08-25 11:45:00 IST (UTC+05:30) | `/edr/policies`, `/api/ha-edr/policies/{id}/enforcement` | POL-003 honesty after #48: missing `appliedVersion`/`lastAppliedAt` ⇒ `unavailable` + `applyAckPathAvailable=false` + apply/ack path unavailable note; UI never green “enforced on host”; surfaces lastCheckedAt/drift. Production agent gRPC apply/ack still open. STAGING CANDIDATE — not PRODUCTION READY. |
| 2026-08-25 11:45:00 IST (UTC+05:30) | `/response/quarantine`, `/api/ha-edr/quarantine`, `/api/ha-edr/isolation` | Closed one remaining `RESP-021` depth gap (list freshness honesty only): quarantine and isolation list responses add `snapshotAt` (server read time) + `asOf` (newest row timestamp on the page) in body and `X-Snapshot-At` / `X-As-Of` headers. Quarantine & Containment shows server freshness banner on Endpoint isolation; files toolbar prefers server snapshot. Not cursor/PIT-bound. Remaining RESP-021: enriched evidence/summaries, action history, governed preview/approval/idempotency for restore/delete/release, resumable delivery, full cursor freshness. STAGING CANDIDATE — not PRODUCTION READY. |
