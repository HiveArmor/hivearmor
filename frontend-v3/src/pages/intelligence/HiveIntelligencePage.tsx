/**
 * HiveIntelligencePage — Prompt 13 + HI-05/HI-07
 * Tabs: Look up | Indicators | Feeds | Ask Hive | Findings
 */
...
import { AskHivePanel } from '@/components/intelligence/AskHivePanel';
...
import { intelligenceFindingService } from '@/services/intelligenceFinding.service';
import {
...
import type { ThreatFeedDTO, TlpLevel } from '@/types/threatIntel.types';
...
type IntelligenceTab = 'lookup' | 'indicators' | 'feeds' | 'ask-hive' | 'findings';
...
export function HiveIntelligencePage(): JSX.Element {
  ...
  const [activeTab, setActiveTab] = useState<IntelligenceTab>('lookup');
  ...
  const iocsQuery = useQuery({
    ...
    queryFn: ({ signal }) =>
      threatIntelService.searchIocsPage(
        { feedId: selectedFeed?.id, page: iocPage, size: IOC_PAGE_SIZE },
        signal
      ),
    enabled: hasRequiredRole && !!selectedFeed && activeTab === 'indicators',
  });

  const findingsQuery = useQuery({
    queryKey: ['intelligence-findings', findingsPage],
    queryFn: ({ signal }) =>
      intelligenceFindingService.listFindings(findingsPage, FINDINGS_PAGE_SIZE, signal),
    enabled: hasRequiredRole && activeTab === 'findings',
  });
  ...
  const iocs = iocsQuery.data?.items;
  const iocTotal = iocsQuery.data?.total ?? 0;
      ...
      {enrichMutation.isSuccess && enrichMutation.data && (
        <div className="hi-result" role="region" aria-label="IOC lookup result">
          ...
          {enrichMutation.data.summary.verdict === 'unknown' && !enrichMutation.data.detail && (
            <p className="hi-honesty">No matching indicator in configured feeds for this value.</p>
          )}
          <div className="hi-result__meta">
            ...
            {enrichMutation.data.summary.tlp && (
              <div>
                TLP:{' '}
                <TlpBadge
                  tlp={enrichMutation.data.summary.tlp.toUpperCase() as TlpLevel}
                  size="sm"
                />
              </div>
            )}
            {enrichMutation.data.summary.firstSeen && (
              <div>First seen: {new Date(enrichMutation.data.summary.firstSeen).toLocaleString()}</div>
            )}
    ...
    <section className="hi-ioc-browser" aria-label="Feed indicator browser">
      {!selectedFeed ? (
        ...
        <>
          ...
          {!iocsQuery.isLoading && !iocsQuery.isError && iocs && iocs.length === 0 && (
            ...
            <div className="hi-ioc-table-wrap">
              ...
              <div className="hi-pager" role="navigation" aria-label="IOC pagination">
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button
                    type="button"
                    disabled={iocPage === 0 || iocsQuery.isLoading}
                  ...
                  <button
                    type="button"
                    disabled={!iocHasMore || iocsQuery.isLoading}
  ...
  const feedsPanel = (
    <section className="hi-panel" aria-label="Threat feeds">
      ...
      {!feedsQuery.isLoading && !feedsQuery.isError && (!feeds || feeds.length === 0) && (
          ...
          <div className="hi-feed-list">
            {filteredFeeds?.map((feed) => (
              <div
                key={feed.id}
                className={
                  selectedFeed?.id === feed.id ? 'hi-feed-card hi-feed-card--selected' : 'hi-feed-card'
                }
                role="button"
                tabIndex={0}
                onClick={() => {
              ...
              >
                <div className="hi-feed-card__top">
                  ...
                  {hasAdminRole ? (
                    ...
                    <span className={feed.enabled ? 'hi-feed-status--on' : 'hi-feed-status--off'}>
                ...
                <div className="hi-feed-card__meta">
                  ...
                  {feed.lastSyncStatus && feed.lastSyncStatus !== 'active' && (
                    <span>Sync: {feed.lastSyncStatus}</span>
                  )}
                  {feed.lastUpdated ? (
  ...
  const findingsPanel = (
    <section className="hi-panel" aria-label="Saved intelligence findings">
      <div className="hi-panel__head">
        <h2>Findings</h2>
        <p>Persisted structured findings with facts vs inference separation.</p>
      </div>

      {findingsQuery.isLoading && <div className="hi-honesty">Loading findings…</div>}
      {findingsQuery.isError && (
        <div className="hi-honesty hi-honesty--error" role="alert">
          Failed to load intelligence findings.
        </div>
      )}
      {findingsQuery.data && findingsQuery.data.items.length === 0 && (
        <div className="hi-honesty">
          No saved findings yet. Ask Hive with persist enabled or create via API.
        </div>
      )}
      {findingsQuery.data && findingsQuery.data.items.length > 0 && (
        <div className="hi-findings-list">
          {findingsQuery.data.items.map((finding) => (
            <IntelligenceFindingCard key={finding.id ?? finding.title} finding={finding} compact />
          ))}
          <div className="hi-pager">
            <span>
              Page {findingsPage + 1} · {findingsQuery.data.total} total
            </span>
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                type="button"
              ...
              <button
                type="button"
                disabled={(findingsPage + 1) * FINDINGS_PAGE_SIZE >= findingsQuery.data.total}
  ...
  return (
    <section className="hi-page" aria-label="Hive Intelligence">
      ...
      <div className="hi-page__stats" role="region" aria-label="IOC inventory summary">
        {statsQuery.isLoading && <span>Loading IOC stats…</span>}
        {statsQuery.isError && (
        ...
        {!statsQuery.isLoading && !statsQuery.isError && statsQuery.data && (
          <>
            <span>
              Active IOCs: <strong>{formatStatCount(statsQuery.data.totalActive)}</strong>
            </span>
            <span>
              IPs: <strong>{formatStatCount(statsQuery.data.byType?.ip)}</strong>
            </span>
            <span>
              Domains: <strong>{formatStatCount(statsQuery.data.byType?.domain)}</strong>
            </span>
            <span>
              Hashes: <strong>{formatStatCount(statsQuery.data.byType?.hash)}</strong>
            </span>
            <span>
              Expired today: <strong>{formatStatCount(statsQuery.data.expiredToday)}</strong>
            </span>
          </>
        )}
        {!hasAdminRole && (
      ...
      <div className="hi-page__tabs">
        <HaTabs
          activeKey={activeTab}
          onSelect={(key) => setActiveTab(key as IntelligenceTab)}
          tabs={[
            { key: 'lookup', title: 'Look up', content: lookupPanel },
            { key: 'indicators', title: 'Indicators', content: indicatorsPanel },
            { key: 'feeds', title: 'Feeds', content: feedsPanel },
            {
              key: 'ask-hive',
              title: 'Ask Hive',
              content: (
            ...
            { key: 'findings', title: 'Findings', content: findingsPanel },
          ]}
        />
      </div>
    </section>
