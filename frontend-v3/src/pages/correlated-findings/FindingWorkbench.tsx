  ...
  return (
    <div className="finding-relationship-map">
      <svg viewBox="0 0 100 100" role="img" aria-labelledby={`relationship-map-title-${finding.id}`}>
        ...
        <defs>
          <filter id={`map-glow-${finding.id}`} x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="1.4" /></filter>
        </defs>
        ...
        {finding.relationshipNodes.map((node) => {
          ...
          return (
            <g key={node.id} className="finding-relationship-map__node" data-type={node.type} data-severity={node.severity ?? undefined}>
              {node.type === 'finding' && <polygon points={points} className="finding-relationship-map__glow" filter={`url(#map-glow-${finding.id})`} />}
    ...
    <article className="finding-workbench" data-compact={compact} aria-labelledby={`finding-title-${finding.id}`}>
      <header className="finding-workbench__header">
        ...
        <div className="finding-workbench__actions">
          {finding.incident ? <Link to={`/incidents/${encodeURIComponent(finding.incident.id)}`} className="finding-workbench__incident"><CheckCircle2 size={14} />{finding.incident.id}</Link> : onPromote && <button type="button" onClick={onPromote} disabled={!promotion?.allowed} title={promotion?.reason ?? 'Preview and promote this finding to an incident'}><ShieldAlert size={14} />Promote</button>}
      ...
      <div className="finding-workbench__body">
        {tab === 'story' && (
          <div className="finding-story" role="tabpanel" aria-label="Attack story">
            <section className="finding-story__narrative">
              ...
              <footer>Generated {new Date(finding.narrative.generatedAt).toLocaleString()} · {finding.narrative.confidence}% narrative confidence</footer>
            </section>
        ...
        {tab === 'evidence' && (
          <section className="finding-evidence" role="tabpanel" aria-label="Supporting evidence">
            <header><div><span>Supporting alerts</span><h3>Signals contributing to this finding</h3><p>Ordered chronologically to preserve the attack story. Open an alert for its full evidence and response context.</p></div><strong>{finding.alertCount}</strong></header>
            <div className="finding-evidence__table" role="table" aria-label="Correlated alerts">
              ...
              {finding.signals.map((signal) => <div role="row" key={signal.id}><time role="cell">{new Date(signal.detectedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</time><span role="cell" data-severity={signal.severity}>{getSeverityLabel(signal.severity)}</span><div role="cell"><strong>{signal.title}</strong><code>{signal.ruleName}</code></div><span role="cell">{signal.entityLabel}</span><span role="cell"><strong>{signal.tactic ?? '—'}</strong><code>{signal.technique ?? '—'}</code></span><Link role="cell" to={`/alerts/${encodeURIComponent(signal.alertId)}`} aria-label={`Open ${signal.title}`}><ExternalLink size={13} /></Link></div>)}
            </div>
            {finding.alertCount > finding.signals.length && <footer><AlertTriangle size={13} />{finding.alertCount - finding.signals.length} additional alerts load only in the complete backend evidence projection.</footer>}
          </section>
        )}

        {tab === 'relationships' && (
          <section className="finding-relationships" role="tabpanel" aria-label="Correlation relationships">
            <div className="finding-relationships__canvas"><header><div><span>Relationship canvas</span><h3>Signals, pivots, and shared entities</h3></div><Radar size={15} /></header><RelationshipMap finding={finding} /></div>
            <aside className="finding-relationships__provenance"><header><div><span>Provenance</span><h3>Correlation controls</h3></div><ShieldAlert size={15} /></header><dl><div><dt>Engine</dt><dd>{finding.correlationEngine.version}</dd></div><div><dt>Evaluated</dt><dd>{new Date(finding.correlationEngine.evaluatedAt).toLocaleString()}</dd></div><div><dt>Rules</dt><dd>{finding.correlationEngine.ruleIds.length}</dd></div><div><dt>Completeness</dt><dd>{finding.dataCompleteness}</dd></div><div><dt>Version</dt><dd>{finding.version}</dd></div></dl><ul>{finding.correlationEngine.ruleIds.map((rule) => <li key={rule}><code>{rule}</code></li>)}</ul><p><Clock3 size={12} />Relationship confidence is evidence strength, not attack certainty. Preserve source evidence for analyst verification.</p></aside>
          </section>
        )}
      </div>
    </article>
