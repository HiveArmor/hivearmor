...
async function listLive(): Promise<ReportingInventory> {
  ...
  const reports: OperationalReport[] = reportsResult.status === 'fulfilled' ? reportsResult.value.map((item) => ({
    id:String(item.id), title:item.repName, type:normalizeType(item.repType), status:item.repUrl?'ready':'review', classification:'Internal', scope:'Authorized tenant scope not reported by legacy API', period:'Snapshot period not reported', generatedAt:item.modificationDate??item.creationDate, generatedBy:item.modificationUser??item.creationUser, version:1, format:'PDF', evidenceCount:0, incidentCount:0, redactionProfile:'Not reported', approval:'pending', freshness:'unknown',
  })) : [];
