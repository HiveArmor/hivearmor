  ...
  const saveRule = useCallback(async (publish: boolean): Promise<void> => {
    ...
    if (publish) {
      ...
      if (!validation.authoritative || !validation.valid) {
        setActionMessage(validation.authoritative ? 'Resolve validation errors before publishing.' : 'Publishing requires the authoritative DET-011 validation contract.');
        return;
      }
    }
    setIsSaving(true);
      ...
      <main className="rule-editor-workspace">
        ...
        <aside className="rule-editor-inspector" aria-label="Rule readiness inspector">
          ...
          {inspectorView === 'validation' && <div className="rule-editor-inspector__scroll">
            ...
            {!effectiveValidation.available && <section className="rule-editor-contract-state"><AlertTriangle size={14} /><p><strong>Authoritative validation unavailable.</strong> The editor is showing local structural checks. Publishing remains blocked until DET-011 is implemented.</p></section>}
