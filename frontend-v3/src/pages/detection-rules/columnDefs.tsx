...
function StatusCell({ data, disabled, loading, onToggle }: { data?: DetectionRule; disabled: boolean; loading: boolean; onToggle: (rule: DetectionRule) => void }): JSX.Element {
  ...
  return (
    <button
      ...
      title={disabled ? MANAGE_DENIED_TITLE : undefined}
...
const MANAGE_DENIED_TITLE = 'Required permission: SOC Manager or Platform Administrator';
...
function ActionsCell({ data, canManage, onDelete, onNavigate, userRole }: ActionsCellProps): JSX.Element | null {
  if (!data) return null;
  return (
    <div className="detection-rule-actions">
      <button type="button" disabled={!canManage} aria-label={`Edit ${data.ruleName}`} title={canManage ? 'Edit rule' : MANAGE_DENIED_TITLE} onClick={(event) => { event.stopPropagation(); onNavigate(`/detection-rules/${data.id}/edit`); }}><Pencil size={14} /></button>
      <button type="button" disabled={!canManage} aria-label={`Test ${data.ruleName}`} title={canManage ? 'Test rule' : MANAGE_DENIED_TITLE} onClick={(event) => { event.stopPropagation(); onNavigate(`/detection-rules/${data.id}/test`); }}><TestTube size={14} /></button>
      {userRole === 'ROLE_ADMIN'
...
export function createColumnDefs(
  ...
  return [
    ...
    { colId: 'actions', headerName: 'Actions', width: 112, sortable: false, filter: false, pinned: 'right', cellRenderer: ({ data }: { data?: DetectionRule }) => <ActionsCell data={data} userRole={userRole} canManage={canManage} onDelete={onDelete} onNavigate={onNavigate} /> },
  ];
}
