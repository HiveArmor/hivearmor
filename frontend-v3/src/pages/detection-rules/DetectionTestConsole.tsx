...
import { detectionRulesFixtureMode, testDetectionSandbox } from './detectionRules.service';
import type { DetectionRule, DetectionSandboxResult } from './detectionRules.types';
  ...
  return (
    <section className="detection-test-console" aria-label="Detection rule test console">
      <div className="detection-test-console__controls">
        ...
        <button type="button" className="detection-test-console__history" disabled title="Requires DET-011 historical preview"><Clock3 size={14} /> Historical preview</button>
        {running ? <button type="button" className="detection-test-console__cancel" onClick={() => controllerRef.current?.abort()}><Square size={13} /> Cancel</button> : <button type="button" className="detection-primary-button" onClick={() => void runTest()}><Play size={14} /> Run test</button>}
      </div>

      <div className="detection-test-console__boundary" role="status"><TestTube2 size={14} /><strong>Safe test boundary.</strong><span>{detectionRulesFixtureMode ? 'Fictional event samples are isolated from production metrics.' : isSigma ? 'The authoritative Sigma evaluator creates no alerts, incidents, notifications, or response actions.' : 'Native CEL single-event execution requires the event-processor sandbox contract; use historical preview for authoritative testing.'}</span></div>
      ...
      <div className="detection-test-console__workspace">
        ...
        <aside className="detection-test-result" aria-live="polite">
          <header><div><strong>Evaluation</strong><span>validation and match trace</span></div>{running && <span className="detection-test-result__running">Running</span>}</header>
          ...
          {running && <section className="detection-test-result__progress"><span /><span /><span /><p>Evaluating normalized fields and condition paths…</p></section>}
          {!running && !result && !error && <section className="detection-test-result__empty"><CircleSlash2 size={28} /><strong>No test has run</strong><p>Review preflight diagnostics, then run this definition against the selected event.</p></section>}
