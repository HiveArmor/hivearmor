...
import { activateSigmaRule, detectionRulesFixtureMode } from './detectionRules.service';
...
export default function DetectionImportPanel({ existingRules, onClose, onStaged }: DetectionImportPanelProps): JSX.Element {
  ...
  const stage = (): void => {
    ...
    if (activateAfterReview && format === 'sigma') {
      void activateSigmaRule(valid[0]?.sigmaId ?? 0).then((activation) => {
        onStaged(
          `${valid.length} fictional Sigma rule${valid.length === 1 ? '' : 's'} staged. `
            + `activated=${String(activation.activated)} engineLoaded=${String(activation.engineLoaded)} `
            + '(LoadReport must list the rule; reload HTTP 202 is not proof). watchLoop may take ~30s.',
        );
        onClose();
      });
      return;
