  ...
  return (
    <section className="exp-page" data-fixture={exposureFixtureMode || undefined} aria-label="Exposure management">
      ...
      <section className="exp-summary" aria-label="Exposure summary">
        <button type="button" data-tone={riskTone(summary?.exposureScore)} onClick={() => setView('attack_paths')}><span><Activity size={12} />Exposure score</span><strong>{summary?.exposureScore ?? '—'}<small>/100</small></strong><em>cross-domain effective risk</em></button>
        <button type="button" data-tone="critical" onClick={() => { setView('attack_paths'); setRisk('critical'); }}><span><Route size={12} />Active attack paths</span><strong>{summary?.activeAttackPaths ?? '—'}</strong><em>entry point to impact</em></button>
        <button type="button" data-tone="critical" onClick={() => setView('critical_assets')}><span><Target size={12} />Critical assets at risk</span><strong>{summary?.criticalAssetsAtRisk ?? '—'}</strong><em>reachable crown jewels</em></button>
        <button type="button" data-tone="high" onClick={() => { setView('attack_paths'); setScope('external'); }}><span><Globe2 size={12} />Internet entry points</span><strong>{summary?.internetEntryPoints ?? '—'}</strong><em>validated external exposure</em></button>
        <button type="button" data-tone="warning" onClick={() => setView('choke_points')}><span><Crosshair size={12} />Choke points</span><strong>{summary?.chokePoints ?? '—'}</strong><em>shared weak points</em></button>
        <button type="button" data-tone="info" onClick={() => setView('remediation')}><span><Wrench size={12} />Reducible paths</span><strong>{summary?.reduciblePaths ?? '—'}</strong><em>priority control changes</em></button>
      </section>
      <section className="exp-operations"><nav className="exp-tabs" aria-label="Exposure views">{VIEWS.map(({ value, label, icon: Icon }) => <button key={value} type="button" data-active={view === value} aria-pressed={view === value} onClick={() => setView(value)}><Icon size={13} />{label}</button>)}</nav><div className="exp-toolbar"><label className="exp-search"><Search size={14} /><input ref={searchRef} value={searchDraft} onChange={(event) => setSearchDraft(event.target.value)} placeholder="Find path, asset, identity, IP, or control…" aria-label="Search exposure records" /><kbd>/</kbd></label><HaCompactSelect ariaLabel="Filter by risk" label="Risk" value={risk} options={RISKS} onChange={setRisk} />{view === 'attack_paths' && <><HaCompactSelect ariaLabel="Filter by exposure scope" label="Scope" value={scope} options={SCOPES} onChange={setScope} /><HaCompactSelect ariaLabel="Filter by path state" label="State" value={state} options={STATES} onChange={setState} /></>}<HaCompactSelect ariaLabel="Filter by calculation window" label="Window" value={timeRange} options={WINDOWS} onChange={setTimeRange} /><span className="exp-auth"><KeyRound size={12} />Authorized topology</span><span className="exp-snapshot">Snapshot {formatTime(query.data?.snapshotAt)}</span></div></section>
