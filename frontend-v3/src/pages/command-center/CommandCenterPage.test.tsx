...
const serviceMocks = vi.hoisted(() => ({ getAlertSummary: vi.fn(), getAlertTimeline: vi.fn(), getIncidents: vi.fn() }));

vi.mock('./commandCenter.service', () => ({ getAlertSummary: serviceMocks.getAlertSummary, getAlertTimeline: serviceMocks.getAlertTimeline }));
vi.mock('@/services/incidents.service', () => ({ getIncidents: serviceMocks.getIncidents }));
...
function renderDashboard(): void {
  ...
  render(<QueryClientProvider client={queryClient}><MemoryRouter><CommandCenterPage /></MemoryRouter></QueryClientProvider>);
}
...
describe('CommandCenterPage', () => {
  beforeEach(() => {
    ...
    serviceMocks.getAlertTimeline.mockResolvedValue([{ hour: '2026-08-18T16:00:00.000Z', low: 0, medium: 4, high: 0 }]);
  ...
  it('renders loading metrics before queries settle', () => {
    ...
    const { container } = render(<QueryClientProvider client={new QueryClient()}><MemoryRouter><CommandCenterPage /></MemoryRouter></QueryClientProvider>);
  ...
  it('renders priority work from the real incident response', async () => {
    serviceMocks.getIncidents.mockResolvedValue({
      total: 1,
      items: [{ id: 12, title: 'Suspicious privileged-account login', severity: 'CRITICAL', status: 'OPEN', assignee: null, createdAt: '2026-08-02T05:30:00Z', updatedAt: '2026-08-02T05:30:00Z', closedAt: null, slaDueAt: '2026-08-02T06:30:00Z', alertCount: 3, evidenceCount: 4, noteCount: 1, description: '', tenant: { id: 1, name: 'Northwind Financial' }, mitreTechniques: [] }],
    });
    renderDashboard();
  ...
  it('renders a full failure with a retry action', async () => {
    ...
    renderDashboard();
    await waitFor(() => expect(screen.getByText('Operational data could not be loaded. Verify connectivity and try again.')).toBeVisible());
