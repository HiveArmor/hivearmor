/**
 * EndpointTimelineBody — the reusable EDR timeline body (SPEC-03, W3).
 *
 * This is the timeline experience (filter bar + scatter chart + event grid +
 * detail drawer + process-tree modal) extracted from `EndpointTimelinePage` so
 * it can be folded into the consolidated agent-detail page as its "Logs /
 * Telemetry" tab WITHOUT the standalone page's `100vh` chrome.
 *
 * Differences from the standalone page:
 *   - `agentId` is a PROP, not read from `useParams` (the tab supplies it).
 *   - Fills its container (`height: 100%`) instead of the viewport (`100vh`).
 *   - The page-level `<h1>` header is optional (`showHeader`, default false) —
 *     the agent-detail page already owns the persistent header.
 *
 * The standalone `EndpointTimelinePage` now renders this body with its own
 * header, so the existing `/edr/timeline/:agentId` route is unchanged (W4 owns
 * retiring it). MITRE context is deliberately NOT shown on EDR events (audit
 * Q5) — the agent-detail Logs tab renders the honesty note around this body.
 *
 * Constraints (unchanged from the page): no `any`, no raw hex, no `var(--ha-*)`
 * into ECharts, no absolute backend URLs.
 */
...
function ProcessTreeModal({ agentId, event, onClose }: ProcessTreeModalProps): JSX.Element {
  ...
  return (
    <HaModal isOpen onClose={onClose} title="Process Tree" width={800}>
      <div style={{ minHeight: 400 }}>
        <ProcessTree processes={roots} isLoading={isLoading} isError={isError} />
      </div>
    </HaModal>
  );
}
...
function DetailDrawer({ agentId, event, onClose }: DetailDrawerProps): JSX.Element {
  ...
  return (
    <>
      ...
      {processTreeOpen && (
        <ProcessTreeModal agentId={agentId} event={event} onClose={() => setProcessTreeOpen(false)} />
      )}
    </>
  );
}
...
export interface EndpointTimelineBodyProps {
  /** The agent whose timeline to show. Supplied by the route param or the parent tab. */
  agentId: string;
    ...
    >
      {showHeader && (
        ...
        >
          ...
          <h1 style={{ fontSize: 'var(--ha-text-xl)', color: 'var(--ha-text-primary)', margin: 0, fontWeight: 600 }}>
            Endpoint Timeline
          </h1>
          {agentId && (
            <span
      ...
      >
        ...
        <TimelineChart events={events} onEventClick={handleEventClick} isLoading={isLoading} />
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, padding: '0 24px 24px', paddingTop: 12, position: 'relative' }}>
        ...
        {!isLoading && !isError && events.length === 0 && (
          ...
          >
            ...
            <p style={{ margin: 0, fontSize: 'var(--ha-text-md)' }}>No events found in the selected range</p>
        ...
        {selectedEvent && (
          <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: 400 }}>
            <DetailDrawer agentId={agentId} event={selectedEvent} onClose={handleCloseDrawer} />
          </div>
        )}
      </div>
