...
import com.hivearmor.util.exceptions.UtmEntityCreationException;
...
public class UtmDashboardResource {
    ...
    /**
     * POST  /ha-dashboards : Create a new utmDashboard.
     *
    ...
    public ResponseEntity<UtmDashboard> createUtmDashboard(@Valid @RequestBody UtmDashboard utmDashboard) {
        ...
        try {
            ...
            return ResponseEntity.created(new URI("/api/ha-dashboards/" + result.getId())).headers(
    ...
    /**
     * PUT  /ha-dashboards : Updates an existing utmDashboard.
     *
    ...
    /**
     * GET  /ha-dashboards/:id : get the "id" utmDashboard.
     *
    ...
    /**
     * DELETE  /ha-dashboards/:id : delete the "id" utmDashboard.
     *
     * @param id the id of the utmDashboard to delete
     * @return the ResponseEntity with status 200 (OK)
     */
    @DeleteMapping("/ha-dashboards/{id}")
    ...
    @PutMapping("/ha-dashboards/sidebar-order")
    public ResponseEntity<Void> updateSidebarOrder(@RequestBody List<SidebarOrderItem> items) {
        final String ctx = CLASSNAME + ".updateSidebarOrder";
        try {
            for (SidebarOrderItem item : items) {
                utmDashboardService.findOne(item.getId()).ifPresent(dashboard -> {
                    dashboard.setSidebarOrder(item.getSidebarOrder());
                    utmDashboardService.save(dashboard);
                });
            }
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .headers(HeaderUtil.createFailureAlert(null, null, msg)).build();
        }
    }

    @PostMapping("/ha-dashboards/import")
    ...
    public static class SidebarOrderItem {
        @NotNull
        private Long id;
        @NotNull
        ...
        public Integer getSidebarOrder() { return sidebarOrder; }
        public void setSidebarOrder(Integer sidebarOrder) { this.sidebarOrder = sidebarOrder; }
    }
