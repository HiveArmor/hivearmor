...
public interface UtmCorrelationRulesRepository extends JpaRepository<UtmCorrelationRules, Long>, JpaSpecificationExecutor<UtmCorrelationRules> {
    @Query(nativeQuery = true, value = "SELECT nextval('hive_correlation_rules_id_seq')")
    Long getNextId();

    @Query(value = "SELECT DISTINCT cr FROM UtmCorrelationRules cr " +
            "LEFT JOIN cr.dataTypes dt " +
            "WHERE " +
            "(:ruleName IS NULL OR (cr.ruleName LIKE :ruleName OR lower(cr.ruleName) LIKE lower(cast(:ruleName AS string)))) " +
            ...
            "(cr.ruleName LIKE :ruleSearch OR lower(cr.ruleName) LIKE lower(cast(:ruleSearch AS string)))) " +
            "AND (:techniqueSearch IS NULL OR cr.ruleTechnique LIKE :techniqueSearch) " +
