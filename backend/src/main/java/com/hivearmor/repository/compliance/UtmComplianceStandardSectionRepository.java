...
public interface UtmComplianceStandardSectionRepository extends JpaRepository<UtmComplianceStandardSection, Long>, JpaSpecificationExecutor<UtmComplianceStandardSection> {
    ...
    @Query(nativeQuery = true, value = "select * from hive_compliance_standard_section css where css.id in (select crc.standard_section_id from hive_compliance_report_config crc) and css.standard_id = :standardId")
