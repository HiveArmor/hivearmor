...
/**
 * SSE broadcaster for tenant-scoped correlated finding updates (COR-006).
 *
 * <p>Manages per-tenant SSE emitter registrations and broadcasts events
 * to all connected clients watching the correlated findings queue. Implements:
 * <ul>
 ...
 *   <li>Monotonic event ID counter per tenant</li>
 *   <li>30-second keepalive via @Scheduled</li>
 *   <li>Last-Event-ID replay from in-memory buffer (last 100 events per tenant)</li>
 *   <li>30-minute SseEmitter timeout</li>
 * </ul>
 *
 * <p>Event types:
 * <ul>
 *   <li>{@code finding.created} — new correlated finding discovered</li>
 *   <li>{@code finding.updated} — finding status/assignment/severity changed</li>
 *   <li>{@code finding.escalated} — finding promoted to incident</li>
 *   <li>{@code finding.stage_added} — new stage appended to existing finding</li>
 *   <li>{@code finding.signal_added} — new signal linked to finding</li>
 * </ul>
 *
 * <p>Sprint 44 — Correlated Findings.
 */
@Service
public class FindingSseService {

    private static final Logger log = LoggerFactory.getLogger(FindingSseService.class);
    private static final String CLASSNAME = "FindingSseService";
    ...
    public FindingSseService(ObjectMapper objectMapper) {
    ...
    public void register(Long tenantId, SseEmitter emitter) {
        ...
        list.add(emitter);
        log.debug("{}: registered emitter for tenant {} (total: {})",
     ...
     *
     * @param tenantId  the tenant identifier
     * @param eventType the SSE event type (e.g., "finding.created", "finding.updated")
    ...
    public void broadcast(Long tenantId, String eventType, Map<String, Object> data) {
        ...
        if (list == null || list.isEmpty()) {
            log.debug("{}: no emitters registered for tenant {}", CLASSNAME, tenantId);
            return;
        }
    ...
    /**
     * Sends keepalive comments to all active emitters every 30 seconds.
     */
