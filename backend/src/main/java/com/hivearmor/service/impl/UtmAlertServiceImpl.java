...
import com.hivearmor.domain.shared_types.static_dashboard.CardType;
import com.hivearmor.repository.UtmAlertLastRepository;
...
import com.hivearmor.util.UnicodeReplacer;
...
public class UtmAlertServiceImpl implements UtmAlertService {
    ...
    public List<CardType> countAlertsByStatus(List<FilterType> filters) throws DashboardOverviewException {
        ...
        try {
            ...
            parse.forEach(agg -> {
                try {
                    Integer statusCode = Integer.parseInt(agg.getKey());
                    CardType card = alertStatus.get(statusCode);
                    if (card != null) {
                        card.setValue((int) agg.getDocCount());
                    } else {
                        log.warn("{}: Unexpected status code {} in aggregation results", ctx, statusCode);
                    }
                } catch (NumberFormatException e) {
                    log.warn("{}: Cannot parse status key as integer: {}", ctx, agg.getKey());
                }
            });
            return new ArrayList<>(alertStatus.values());
        } catch (Exception e) {
