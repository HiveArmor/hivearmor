package com.hivearmor.service.index_policy;
...
public class IndexPolicyService {
    ...
    private final String SNAPSHOT_REPOSITORY_NAME = "hivearmor_backups";
    ...
    public void init() throws Exception {
        ...
        try {
            // 1. Creating the HiveArmor main index management policy if it does not exist
            createPolicy();
        } catch (Exception e) {
            // Non-fatal in dev: ISM policy creation may fail when OpenSearch is unavailable
            // or index patterns use a naming scheme not accepted by the ISM API (e.g. leading '_').
            log.warn(ctx + ".createPolicy (non-fatal): " + e.getMessage());
        }

        // 2. Registering snapshots repository — non-fatal (may fail in dev with self-signed certs)
        try {
            registerSnapshotRepository();
        } catch (Exception e) {
            log.warn(ctx + ".registerSnapshotRepository (non-fatal): " + e.getMessage());
        }
    }
                ...
                .withPolicy(Policy.builder()
                    ...
                    .withState(State.builder()
                        ...
                        .executeAction(Action.builder()
                            .snapshot(new ActionSnapshot(SNAPSHOT_REPOSITORY_NAME, "hivearmor_snapshot"))
                            .build())
                        .transitionTo(Transition.builder()
                            .stateName(Constants.STATE_OPEN)
                            .build())
