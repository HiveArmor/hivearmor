...
```bash
# Linux / macOS
sudo ./hivearmor_agent_service install <server_address> no \
  --enrollment-token-file /secure/operator/path/enrollment.token

# Windows (Administrator PowerShell)
.\hivearmor_agent_service.exe install <server_address> no --enrollment-token-file -
```
...
| `--enrollment-token-file <path|->` | One-time, tenant-bound enrollment token read from an owner-only file or standard input; never pass it as a positional argument |
...
3. Atomically consumes the one-time token, registers the agent with AgentManager, and saves its device credential in an authenticated owner-only envelope
...
agent-id: <integer>
agent-key: <versioned AES-256-GCM envelope>
insecure: false
```

The `agent-key` field uses a versioned AES-256-GCM envelope with an HKDF-SHA256 key derived from the build-time wrapping secret and a per-installation UUID. Both files are written atomically with owner-only permissions on POSIX systems. Legacy configurations remain read-only compatible and are upgraded on the next save. Do not edit either file manually.
...
Commands:
  install <server> <insecure>            Install using --enrollment-token-file <path|->
  rotate-credential                      Validate and install --credential-file <path|->
...
- The agent communicates over TLS with certificate validation enabled by default. Production requires a trusted CA and hostname verification; the development-only validation bypass must not be used.
- One-time enrollment and rotated device credentials are accepted only through protected files or standard input, never process arguments.
- The `agent-key` credential is sealed with AES-256-GCM using an HKDF-SHA256-derived per-installation key. The plaintext key never touches disk or routine update responses.
