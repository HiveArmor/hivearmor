...
func checkConnection(url string, ctx context.Context) error {
	...
	defer func() {
		err := resp.Body.Close()
		if err != nil {
			_ = catcher.Error("error closing response body: %v", err, map[string]any{"process": "plugin_com.hivearmor.o365"})
		}
	}()
...
func infiniteRetryIfXError(f func() error, exception string) error {
	...
	for {
		...
		if err != nil && is(err, exception) {
			if !xErrorWasLogged {
				_ = catcher.Error("An error occurred (%s), will keep retrying indefinitely...", err, map[string]any{"process": "plugin_com.hivearmor.o365"})
