...
import (
	...
	"github.com/hivearmor/plugins/sophos/config"
)
...
func main() {
	mode := plugins.GetCfg("plugin_com.hivearmor.sophos").Env.Mode
	if mode != "manager" {
		return
	}
	...
	for t := 0; t < 2*runtime.NumCPU(); t++ {
		go plugins.SendLogsFromChannel("com.hivearmor.sophos")
	}
...
func syncActiveGroups(newConfig *config.ConfigurationSection) {
	...
	if newConfig == nil || !newConfig.ModuleActive {
		catcher.Info("Module deactivated, clearing all groups", map[string]any{
			"process": "plugin_com.hivearmor.sophos",
		})
		activeGroups = make(map[int32]*config.ModuleGroup)
		return
	}
	...
	for {
		...
		case <-ticker.C:
			...
			if err := connectionChecker(urlCheckConnection); err != nil {
				_ = catcher.Error("External connection failure detected", err, map[string]any{"process": "plugin_com.hivearmor.sophos"})
				continue
			}
...
func pull(startTime time.Time, group *config.ModuleGroup) {
	...
	if err != nil {
		_ = catcher.Error("error getting logs", err, map[string]any{"process": "plugin_com.hivearmor.sophos"})
		return
	}
...
func (p *SophosCentralProcessor) getAccessToken() (string, error) {
	...
	if err != nil {
		return "", catcher.Error("all retries failed when getting access token", err, map[string]any{"process": "plugin_com.hivearmor.sophos"})
	}
...
func (p *SophosCentralProcessor) getTenantInfo(accessToken string) error {
	...
	if err != nil {
		return catcher.Error("all retries failed when getting tenant info", err, map[string]any{"process": "plugin_com.hivearmor.sophos"})
	}
...
func (p *SophosCentralProcessor) getLogs(fromTime int64, nextKey string) ([]string, string, error) {
	...
	if err != nil {
		return nil, "", catcher.Error("all retries failed when getting access token", err, map[string]any{"process": "plugin_com.hivearmor.sophos"})
	}
	...
	if p.TenantID == "" || p.DataRegion == "" {
		...
		if err != nil {
			return nil, "", catcher.Error("all retries failed when getting tenant info", err, map[string]any{"process": "plugin_com.hivearmor.sophos"})
		}
	}
	...
	for {
		...
		if err != nil {
			return nil, "", catcher.Error("all retries failed when getting logs", err, map[string]any{"process": "plugin_com.hivearmor.sophos"})
		}

		for _, item := range response.Items {
			jsonItem, err := json.Marshal(item)
			if err != nil {
				_ = catcher.Error("error marshalling content details", err, map[string]any{"process": "plugin_com.hivearmor.sophos"})
				continue
			}
