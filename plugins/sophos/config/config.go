...
func StartConfigurationSystem() {
	for {
		...
		if !pluginConfig.Exists() {
			_ = catcher.Error("plugin configuration not found", nil, map[string]any{"process": "plugin_com.hivearmor.sophos"})
	...
	for {
		...
		if err != nil {
			catcher.Error("Failed to connect to server", err, map[string]any{"process": "plugin_com.hivearmor.sophos"})
			cancel()
			time.Sleep(reconnectDelay)
			continue
		}
		...
		if state == connectivity.Shutdown || state == connectivity.TransientFailure {
			catcher.Error("Connection is in shutdown or transient failure state", nil, map[string]any{"process": "plugin_com.hivearmor.sophos"})
			cancel()
			time.Sleep(reconnectDelay)
			continue
		}
		...
		if err != nil {
			catcher.Error("Failed to create stream", err, map[string]any{"process": "plugin_com.hivearmor.sophos"})
			conn.Close()
			cancel()
			time.Sleep(reconnectDelay)
			continue
		...
		if err != nil {
			catcher.Error("Failed to send PluginInit", err, map[string]any{"process": "plugin_com.hivearmor.sophos"})
			conn.Close()
			cancel()
			time.Sleep(reconnectDelay)
			continue
		...
		for {
			...
			if err != nil {
				if strings.Contains(err.Error(), "EOF") {
					catcher.Info("Stream closed by server, reconnecting...", map[string]any{"process": "plugin_com.hivearmor.sophos"})
					conn.Close()
					cancel()
					time.Sleep(reconnectDelay)
					break
				...
				if ok && (st.Code() == codes.Unavailable || st.Code() == codes.Canceled) {
					catcher.Error("Stream error", err, map[string]any{"process": "plugin_com.hivearmor.sophos", "status_message": st.Message()})
					conn.Close()
					cancel()
					time.Sleep(reconnectDelay)
					break
				} else {
					catcher.Error("Stream receive error", err, map[string]any{"process": "plugin_com.hivearmor.sophos"})
					time.Sleep(reconnectDelay)
					continue
				}
			}
			...
			case *BiDirectionalMessage_Config:
				catcher.Info("Received configuration update", map[string]any{"process": "plugin_com.hivearmor.sophos"})
