...
func StartConfigurationSystem() {
	for {
		...
		if !pluginConfig.Exists() {
			_ = catcher.Error("plugin configuration not found", nil, map[string]any{"process": "plugin_com.hivearmor.azure"})
	...
	for {
		...
		if state == connectivity.Shutdown || state == connectivity.TransientFailure {
			_ = catcher.Error("Connection is in shutdown or transient failure state", nil, map[string]any{"process": "plugin_com.hivearmor.azure"})
			cancel()
			time.Sleep(reconnectDelay)
			continue
		}
		...
		if err != nil {
			_ = catcher.Error("Failed to create stream", err, map[string]any{"process": "plugin_com.hivearmor.azure"})
			_ = conn.Close()
			cancel()
			time.Sleep(reconnectDelay)
			continue
		...
		if err != nil {
			_ = catcher.Error("Failed to send PluginInit", err, map[string]any{"process": "plugin_com.hivearmor.azure"})
			_ = conn.Close()
			cancel()
			time.Sleep(reconnectDelay)
			continue
		...
		for {
			...
			if err != nil {
				if strings.Contains(err.Error(), "EOF") {
					catcher.Info("Stream closed by server, reconnecting...", map[string]any{"process": "plugin_com.hivearmor.azure"})
					_ = conn.Close()
					cancel()
					time.Sleep(reconnectDelay)
					break
				...
				if ok && (st.Code() == codes.Unavailable || st.Code() == codes.Canceled) {
					_ = catcher.Error("Stream error: "+st.Message(), err, map[string]any{"process": "plugin_com.hivearmor.azure"})
					_ = conn.Close()
					cancel()
					time.Sleep(reconnectDelay)
					break
				} else {
					_ = catcher.Error("Stream receive error", err, map[string]any{"process": "plugin_com.hivearmor.azure"})
					time.Sleep(reconnectDelay)
					continue
				}
			}
			...
			case *BiDirectionalMessage_Config:
				catcher.Info("Received configuration update", map[string]any{"process": "plugin_com.hivearmor.azure"})
